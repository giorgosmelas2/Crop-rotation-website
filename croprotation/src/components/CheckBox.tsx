...
export const CheckBox: React.FC<CheckBoxProps> = ({ items, selected, setSelected }) => {
  ...
  const handleCheckboxChange = (item: string) => {
    ...
    // If "All" is selected, unselect it
    let newSelected = selected.includes(item)
      ? selected.filter(i => i !== item && i !== "All")
      : [...selected, item];

    // If all items are selected, select "All"
    const cropsExceptAll = items.filter(i => i !== "All");
    if (newSelected.length === cropsExceptAll.length) {
      newSelected = ["All", ...cropsExceptAll];
    }
